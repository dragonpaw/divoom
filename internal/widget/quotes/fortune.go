// fortune(1) cookies — sourced from shlomif/fortune-mod's
// public-domain corpus and parsed with scripts/parse-fortune.py.
// Regenerate when refreshing the pool.
package quotes

// NewFortune returns a Source drawn from the BSD fortune cookie
// database (~500 entries). Mixed attribution: many
// cookies are anonymous, those with a trailing `-- Author` line
// carry their attribution inline using the existing ` — Author`
// convention.
func NewFortune() *Source { return newSource("fortune", fortuneCookies) }

var fortuneCookies = []string{
