...
// returns a deduped, sorted slice of drink IDs. The category list
...
//
// Sorting is by numeric drink ID for stability — a given drink lands
// at the same index across `make push-frame` runs, so the per-drink
// disk cache (JSON + baked JPG) stays aligned with the indexed bg
// path on the device. The scene picks a random index per activation
// anyway (see scene_cocktail.go), so user-visible order remains
// random.
