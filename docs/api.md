...
| Field      | Type   | Notes |
|------------|--------|-------|
| `ID`       | number | >0. **Cached across `EnterCustomControlMode` calls** — see Empirical findings (2026-05-21). Re-installing with the same ID only updates `TextMessage`; `FontSize`, `Height`, `StartY`, etc. retain the first install's value until you call `Device/ExitCustomControlMode`. |
...
| 2026-05-22 | **DO NOT just uncomment `ADB_TRANSPORT_PORT=5555` in `/etc/init.d/adbd` and reboot.** Doing so killed adbd entirely after the next power cycle — frame was unreachable on both USB and TCP. The Divoom app and local `divoom_api` on port 9000 kept working (independent service), so the device wasn't bricked, but adb access was gone until a **factory reset via the Divoom app** wiped the overlay filesystem and restored the original init script. Either this adbd build doesn't honor `ADB_TRANSPORT_PORT` the way the OpenWrt-pattern script suggests, or the conditional branch is buggy on this busybox sh and corrupts adbd's startup. A second attempt MUST test in a way that preserves a recovery path (e.g. push a separate `S99adb-tcp` init script that doesn't touch `/etc/init.d/adbd` itself, or test the file change with a known-no-op first). | Pushed patched `adbd` (one-line diff: uncomment), power-cycled, adbd unreachable on both transports. Recovered via Divoom app's factory reset. |
